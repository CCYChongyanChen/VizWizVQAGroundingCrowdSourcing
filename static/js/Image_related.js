...
$('#WholeImage:checkbox').change(
    function(){
        ...
        if ($(this).is(':checked')) {
            XY_names['xy'+activate_tab]=[{"x":1.5,"y":1.5},{"x":398.5,"y":1.5},{"x":398.5,"y":498.5},{"x":1.5,"y":498.5},{"x":1.5,"y":1.5}];

            finishFlags['finishFlag'+activate_tab]=true; 
            draw_canvas();       
            ControlNext();
        }
        else{
        ...
        // //=========unknown bug====happens when select nodraw, go next, go back, and can select the whole image=====
        // if ($('input[value=N]:checked').length>1 && $('#nodraw:checked').length==0){//////
